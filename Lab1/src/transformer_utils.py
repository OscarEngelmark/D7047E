"""transformer_utils.py — Trainer-based helpers for sentiment classification.

Exports
-------
TransformerSentimentDataset : torch Dataset that tokenizes text for a transformer
...
def build_tf_datasets(
    ...
    label_col: str = "Class",
    max_length: int = 128,
) -> Dict[str, TransformerSentimentDataset]:
    """Build train / val / test datasets for Trainer from DataFrames."""
    return {
        split: TransformerSentimentDataset(
            texts=df[text_col],
            labels=df[label_col],
            tokenizer=tokenizer,
